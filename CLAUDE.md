...
3. `/leagues/[sleeper_league_id]` — the deep view. Clicking "Open league" on any card from #1 or #2 is a plain `<Link>` navigation to the deep view, so the branded loading boundary shows immediately on click. The deep-view page calls `lib/league-pulse.ts pulseLeague` on every render (writes to `leagues`, `rosters`, `league_users`, `league_transactions` via service-role); the 30-minute cache inside `pulseLeague` short-circuits redundant Sleeper hits (power rankings still recompute on the cached path). A sync failure renders a branded retry state (`components/league-load-error.tsx`), not a redirect or 404.
...
- Cache TTL: 30 minutes (`LEAGUE_PULSE_TTL_MS`). `force=true` bypasses; `force=false` (default) skips the Sleeper refetch and returns cached: true if the DB row is fresh (last_pulsed_at within the TTL). The cached path still recomputes power rankings before returning, so power rankings track the nightly player-value syncs even when the league rows are not refetched. Only a stale cache (over 30 minutes) or a league with no DB rows triggers a full Sleeper sync.
...
- ABSOLUTE RULE: League power rankings are recomputed ONLY on demand, when a league deep view loads through `pulseLeague` (on every load, including the cached path), or via the manual `npm run calculate:power-rankings`. NEVER wire per-league power-ranking recomputation into a nightly cron. The nightly `/api/cron/recalculate-derived` job rebuilds only the global `rankings` and `player_value_trends` tables; it must not iterate leagues. Recomputing every stored league nightly does not scale to tens of thousands of leagues, and unviewed leagues never need a cache row.
