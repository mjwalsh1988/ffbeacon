...
## Guides as courses (2026-09-16). New superflex guide, and the FAAB and Positional WAR guides rebuilt in the trade guide's course format

AD-T011 | completed | Guide: Superflex strategy, eight lessons with diagrams, three interactives and one live figure, first person
     | files: app/guides/superflex-strategy/page.tsx, app/guides/superflex-strategy/superflex-figures.tsx, app/guides/superflex-strategy/superflex-classroom.tsx, lib/guides/superflex-price-pairs.ts, lib/guides/superflex-price-pairs.test.ts, lib/guides/published.ts, lib/nav-tree.ts, lib/site.ts, lib/breadcrumbs.ts, app/guides/page.tsx, app/page.tsx, app/api/og/guide/[slug]/route.tsx, app/author/michael/page.tsx, lib/guides/fantasy-football-terms.ts (superflex and 2QB entries link to the guide), components/rankings/rankings-view.tsx (superflex boards link to the guide), app/guides/fantasy-football-trade-guide/page.tsx (one link)
     | notes: Keyword targets from Search Console (the five superflex and TE premium boards at about a thousand impressions a month, positions 8 to 12) and Google autocomplete ("superflex strategy", "how many qbs to roster in superflex", "2qb fantasy football strategy", "te premium fantasy strategy"). The one live figure reads the same cached rankings boards the /rankings pages render, in the reader's resolved source and league type, from ONE source for both formats; every other number is invented and says so. Mechanical claims checked against lib/power-pulse/types.ts, lib/sleeper-to-format.ts, lib/positional-war/replacement.ts and lib/signal-check/values.ts. Not committed: owner review pending.
     | verified: yes. tsc clean; full vitest run passing (338 files, 5,122 tests, including the new price-pairs tests); rendered on the dev server with the live table populated (Josh Allen 1st in dynasty superflex, 14th in dynasty 1QB from FF Beacon); the roster planner, draft-slot plan and checklist respond; the OG card returns a 144 KB PNG; the superflex rankings board carries the guide link. Accessibility review applied (bye grid is a real table, draft-slot live region is its header only, 44px links in the live table, atomic counters, eyebrow no longer aria-hidden). Implementation review applied (start/sit sentence corrected: the implied total sits beside the projection, not inside it; picks priced only in dynasty formats; snake-draft pick gaps corrected to ten and twelve; two negative-parallelism sentences rewritten; two more loader tests). Security review: no findings; its two asides applied (duplicate ternary, generatedAt read hoisted out of the loop).

AD-T012 | completed | Guide refresh: FAAB strategy rebuilt as eight lessons with four diagrams, a bid worksheet and a pre-bid checklist
     | files: app/guides/faab-strategy/page.tsx, app/guides/faab-strategy/faab-figures.tsx, app/guides/faab-strategy/faab-classroom.tsx, lib/guides/published.ts (updatedAt), app/guides/page.tsx (bullets), app/page.tsx (lessons), app/api/og/guide/[slug]/route.tsx (badge)
     | notes: Every paragraph of the original prose and the FAQ kept. Bid bands drawn from lib/faab/default-settings.ts, the dollar calendar from lib/faab/market.ts urgencyMultiplier (0.85 through week 3 rising to 1.40 from week 12, by default), the ladder rungs from the worked example. The worksheet's rival-count shape is invented and labelled so. Implementation review caught that the original worked example (6 points a week, 12 odds points) crossed the all-in thresholds the same page quotes; the example now adds about 3 a week and 8 odds points, so the 27/36/40 ladder is consistent with the rule. Not committed.
     | verified: yes. tsc clean; full vitest run passing; rendered on the dev server; accessibility review applied (atomic counter).

AD-T013 | completed | Guide refresh: Positional WAR explained rebuilt as seven lessons with five diagrams, a move-the-line box and a which-question quiz
     | files: app/guides/positional-war-explained/page.tsx, app/guides/positional-war-explained/war-figures.tsx, app/guides/positional-war-explained/war-classroom.tsx, lib/guides/published.ts (updatedAt), app/guides/page.tsx (bullets), app/page.tsx (lessons), app/api/og/guide/[slug]/route.tsx (badge)
     | notes: Every paragraph of the original prose and the FAQ kept. Mechanism re-checked against lib/positional-war/war.ts and replacement.ts; the marker figure labels RB28 with its real value, not zero. The naming rule holds in every figure title, summary, caption and live region (lib/positional-war/naming.test.ts passes). Not committed.
     | verified: yes. tsc clean; full vitest run passing; rendered on the dev server; accessibility review applied (quiz counter live only on completion; marker figure one heading level down).

AD-T014 | completed | Shared: guide section eyebrow no longer aria-hidden; checklist counters atomic
