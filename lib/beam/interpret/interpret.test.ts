...
import { BEAM_STATS, getStat, statForPosition } from "@/lib/beam/stats/registry";
...
describe("longest-match phrase claiming", () => {
  ...
  it("prefers 'yards per carry' over both 'yards' and 'carry'", () => {
    expect(extractEntities("gibbs yards per carry 2025").statIds).toEqual(["rush_ypc"]);
  });

  it("prefers 'points allowed' over a bare 'points' for a defense", () => {
    expect(extractEntities("ravens points allowed").statIds).toEqual(["pts_allow"]);
  });
});
...
describe("season extraction", () => {
  ...
  it("reads 'this year' as the current season, resolved later", () => {
    expect(extractEntities("purdy yards this year").seasons).toEqual([{ kind: "current" }]);
  });
});
...
describe("capability routing", () => {
  ...
  it("routes a two-player statistic question to the comparison", () => {
    expect(topReading("who had more receiving yards in 2025 ceedee lamb or garrett wilson")).toBe(
      "player.compare.stat",
    );
  });

  it("routes a two-player judgement question to the verdict", () => {
    expect(topReading("who is better for dynasty garrett wilson or drake london")).toBe(
  ...
  it("reads the league type in a draft question as the lens", () => {
    expect(extractEntities("who should i draft in a redraft league amon ra or james cook").lens).toBe(
      "win-now",
    );
    expect(extractEntities("who should i draft in my startup bijan robinson or jahmyr gibbs").lens).toBe(
      "dynasty",
    );
    // No league type stated. The interpreter fills this from the reader's own
    // format, so extraction must report that nobody said.
    expect(extractEntities("who should i draft bijan robinson or jahmyr gibbs").lens).toBeNull();
  });
  ...
  it("routes a whole-season question to the stat line", () => {
    expect(topReading("how did jahmyr gibbs do in 2025")).toBe("player.stat.line");
    ...
    expect(topReading("how is jahmyr gibbs doing in 2025")).toBe("player.stat.line");
  });
  ...
  it("disqualifies a comparison when only one player was named", () => {
    ...
    const scored = scoreCapabilities(CAPABILITIES, entities, settings);
    expect(scored.some((c) => c.capability.id === "player.compare.verdict")).toBe(false);
  });

  it("disqualifies a stat question with no statistic in it", () => {
    const entities = extractEntities("brock purdy");
    const scored = scoreCapabilities(CAPABILITIES, entities, settings);
    expect(scored.some((c) => c.capability.id === "player.season.stat")).toBe(false);
  });
});

describe("name spans", () => {
  it("leaves only the name after every vocabulary has claimed its words", () => {
    const e = extractEntities("how many passing yards did brock purdy have in 2025");
    expect(e.nameSpans.map((s) => s.text)).toEqual(["brock purdy"]);
  });

  it("splits two players around a comparator", () => {
    const e = extractEntities("who had more targets ceedee lamb or garrett wilson");
    expect(e.nameSpans.map((s) => s.text)).toEqual(["ceedee lamb", "garrett wilson"]);
  });
  ...
  it("splits two players joined by 'and' rather than a comparator", () => {
    ...
    const e = extractEntities("who should i draft between amon ra and james cook");
  ...
  it("claims the league-shape words a draft question carries", () => {
    const e = extractEntities("who should i draft in a redraft league amon ra or james cook");
    expect(e.nameSpans.map((s) => s.text)).toEqual(["amon ra", "james cook"]);
    ...
    for (const stat of BEAM_STATS) {
      for (const phrase of stat.phrasings) {
        ...
        if (existing && existing !== stat.id) {
          collisions.push(`"${phrase}" claimed by both ${existing} and ${stat.id}`);
        }
  ...
  it("only lets a stat resolve to positions it actually supports", () => {
    for (const stat of BEAM_STATS) {
      for (const position of stat.resolutionPositions ?? []) {
        expect(stat.positions, `${stat.id} resolution position ${position}`).toContain(position);
      }
    }
  });
});
...
describe("week ranges", () => {
  const range = (question: string) => extractEntities(normalizeText(question)).weeks;

  it("reads every way people write a range", () => {
    expect(range("passing yards between weeks 2 and 8")).toMatchObject({ start: 2, end: 8 });
    expect(range("passing yards from week 2 to week 8")).toMatchObject({ start: 2, end: 8 });
    expect(range("passing yards weeks 2 through 9")).toMatchObject({ start: 2, end: 9 });
    expect(range("passing yards in weeks 2-8")).toMatchObject({ start: 2, end: 8 });
    expect(range("passing yards in week 5")).toMatchObject({ start: 5, end: 5 });
  });
  ...
  it("claims week numbers before the season scan can read them as years", () => {
    ...
    const e = extractEntities(normalizeText("how many points did puka nacua score between weeks 15 and 17 last year"));
  ...
  it("survives a typo in the word 'weeks'", () => {
    ...
    expect(range("passing yards in weeek 5")).toMatchObject({ start: 5, end: 5 });

    const e = extractEntities(
      normalizeText("how many receiving yards did marvin harrison and michael wilson have between weks 10 and 17 of last year"),
    );
    expect(e.nameSpans.map((s) => s.text)).toEqual(["marvin harrison", "michael wilson"]);
    expect(e.seasons).toEqual([{ kind: "relative", offset: -1 }]);
  });
  ...
  it("does not read a word as a week just because it is close to one", () => {
    ...
    expect(range("how many carries did saquon barkley have in 2024")).toBeNull();
  });

  it("does not mistake the lens phrase 'this week' for a range", () => {
    const e = extractEntities(normalizeText("who is better this week ceedee lamb or puka nacua"));
    expect(e.weeks).toBeNull();
    expect(e.lens).toBe("this-week");
  });

  it("does not swallow a name that follows a week number", () => {
    const e = extractEntities(normalizeText("how many yards did purdy have in week 5"));
  ...
  it("routes a week-range question to the plain total, not the projection", () => {
    expect(topReading("how many passing yards did brock purdy have last year between weeks 2 and 8")).toBe(
      "player.season.stat",
    );
    expect(
      topReading(
  ...
  it("routes only an explicit projection to the projection capability", () => {
    expect(
      topReading("project the season total from weeks 2 through week 4 from last year for michael wilson"),
    ).toBe("player.weeks.projection");
    expect(topReading("what pace was puka nacua on from weeks 1 to 6 in 2025")).toBe(
  ...
  it("disqualifies a projection with no week range in it", () => {
    ...
    const entities = extractEntities(normalizeText("project puka nacua for 2025"));
    const scored = scoreCapabilities(CAPABILITIES, entities, settings);
    expect(scored.some((c) => c.capability.id === "player.weeks.projection")).toBe(false);
  });
  ...
  it("claims the words people use to describe a projection's input", () => {
    ...
    const e = extractEntities(
      normalizeText("project tucker krafts season totals for last year by using his weeks 1 through 5"),
    );
  ...
  it("claims 'each' and 'from' so neither becomes a player name", () => {
    const e = extractEntities(
      normalizeText("how many receiving yards did marvin harrison and michael wilson each have from weeks 2 to 9"),
    );
    expect(e.nameSpans.map((s) => s.text)).toEqual(["marvin harrison", "michael wilson"]);
  });
});
...
describe("projections and beat rates", () => {
  it("routes a beat-rate comparison ahead of the head-to-head verdict", () => {
    ...
    expect(topReading("who beats his projection more often james cook or josh jacobs")).toBe(
      "player.compare.reliability",
    );
  });

  it("routes single-player reliability and projection questions", () => {
    expect(topReading("what is james cooks beat rate")).toBe("player.reliability");
    expect(topReading("how reliable is puka nacua against his projections")).toBe(
      "player.reliability",
    );
    expect(topReading("what is bijan robinson projected for")).toBe("player.projection");
    expect(topReading("who is projected higher bijan robinson or jahmyr gibbs")).toBe(
  ...
  it("keeps a week-range projection separate from a rest-of-season one", () => {
    ...
    expect(topReading("what is michael wilsons projected total from weeks 2 to 4 of last year")).toBe(
      "player.weeks.projection",
    );
  });
  ...
  it("reads a multi-season span without eating the season inside it", () => {
    ...
    expect(span("beat rate over the last 3 years")).toMatchObject({ kind: "lastN", count: 3 });
    expect(span("beat rate over the past two seasons")).toMatchObject({ kind: "lastN", count: 2 });
    expect(span("beat rate since 2023")).toMatchObject({ kind: "since", season: 2023 });
    // "last year" is a single season and stays one, handled by the lexicon.
    expect(span("beat rate last year")).toBeNull();
    expect(extractEntities(normalizeText("beat rate last year")).seasons).toEqual([
      { kind: "relative", offset: -1 },
    ]);
  });
  ...
  it("leaves nothing unclaimed in a beat-rate question", () => {
    ...
    expect(e.nameSpans.map((s) => s.text)).toEqual(["james cook", "josh jacobs"]);
...
describe("out-of-scope detection", () => {
  it("flags questions about the reader's own league", async () => {
    ...
    expect(looksOutOfScope("who should i start this week in my league")).toBe(true);
  ...
  it("lets a draft question through once the reader names both players", async () => {
    ...
    expect(looksOutOfScope(question, { comparingNamedPlayers: true })).toBe(false);
  });
