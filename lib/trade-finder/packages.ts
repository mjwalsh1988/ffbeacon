...
function withinBand(total: number, target: number): boolean {
  if (target <= 0) return false;
  return total >= target * (1 - UNDER_TOLERANCE) && total <= target * (1 + OVER_TOLERANCE);
}
...
export function balancePackages(
  target: number,
  pool: AssetRef[],
  opts: { required?: AssetRef | null; maxAssets?: number } = {},
): AssetRef[][] {
  ...
  const consider = (assets: AssetRef[]) => {
    ...
    if (!withinBand(total, target)) return;
  ...
  if (found.length < MAX_PACKAGES && roomForTwo) {
    for (let i = 0; i < rest.length && found.length < MAX_PACKAGES; i += 1) {
      for (let j = i + 1; j < rest.length && found.length < MAX_PACKAGES; j += 1) {
        ...
        if (total > target * (1 + OVER_TOLERANCE)) break;
        consider(combo);
      }
    }
  }
  ...
  if (found.length === 0 && !required && maxAssets >= 3) {
    for (let i = 0; i < rest.length && found.length < MAX_PACKAGES; i += 1) {
      for (let j = i + 1; j < rest.length && found.length < MAX_PACKAGES; j += 1) {
        if (packageValue([rest[i], rest[j]]) > target * (1 + OVER_TOLERANCE)) break;
        for (let k = j + 1; k < rest.length && found.length < MAX_PACKAGES; k += 1) {
          const combo = [rest[i], rest[j], rest[k]];
          if (packageValue(combo) > target * (1 + OVER_TOLERANCE)) break;
          consider(combo);
        }
      }
    }
 ...
 */
export function incomingPairs(pool: AssetRef[], target: number): AssetRef[][] {
  ...
  for (let i = 0; i < players.length && out.length < MAX_PACKAGES; i += 1) {
    for (let j = i + 1; j < players.length && out.length < MAX_PACKAGES; j += 1) {
      ...
      if (withinBand(packageValue(pair), target)) out.push(pair);
    }
  }
  return out;
}
