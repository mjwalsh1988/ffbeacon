...
export default async function BeaconManualPage() {
  ...
  const [composer, active, recompute] = await Promise.all([
    flagshipId
      ? admin
    ...
    admin
      ...
      .select("id, target, player_id, format_config_id, adjustment_type, magnitude, silent, reason, pick_round, pick_position, players(first_name, last_name, position)")
      .eq("is_active", true)
  ...
  const signals: ManualSignalItem[] = (active.data ?? []).map((s) => {
    ...
    const subject = s.target === "player"
      ...
      : `Pick R${s.pick_round} ${s.pick_position ?? ""}`.trim();
    return {
      id: s.id,
      target: s.target,
      subject,
      scope: s.format_config_id ? (slugById.get(s.format_config_id) ?? "format") : "all formats",
  ...
  return (
    <BeaconPageShell
      ...
      description="Owner nudges read only by the FF Beacon engine. Create a signal below, then recompute to see its effect. Silent signals change the value but stay out of the trend chips."
      recompute={recompute}
    >
      <div className="space-y-10">
        <section aria-labelledby="composer-h">
          ...
          <ManualComposer players={players} formats={formats} />
        </section>
