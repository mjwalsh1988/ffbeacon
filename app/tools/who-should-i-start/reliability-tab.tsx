/**
 * The Reliability tab: how often each player in the comparison actually
 * delivers what the projection says.
 *
 ...
 *
 *   Beat rate      Share of projected weeks the player met or beat the number.
 *                  A week a player was projected for but did not play counts as
 *                  a MISS, so an injured stretch drags the number down instead
 *                  of vanishing from the denominator. That is a deliberate
 *                  choice in the nightly calculation and the reason this beat
 *                  rate is lower, and more useful, than the ones you see
 *                  elsewhere.
 *   Availability   Share of projected weeks the player was on the field at all.
 ...
 *
 * Generalised from a fixed pair to two-to-eight players (section 2.7 of
 * docs/seo/who-should-i-start-and-site-seo-plan.md): the tab takes the group's
 * `sides` and its `extras` map directly, in the group's own order, and renders
 * one card plus one strip (one row of the differential chart) per player.
 * Nothing about what is measured or how it is explained changed, only that it
 * now runs over N players instead of two.
 */
...
/**
 * The season the week-by-week charts cover: the first graded season any player
 * in the group carries, in the group's own order.
 *
 ...
 * as a contradiction.
 */
function chartSeasonOf(players: Side[]): number | null {
  for (const s of players) {
    if (s.reliability?.season != null) return s.reliability.season;
  }
  return null;
}

/**
 * Per-player accent color, dash, and marker, one slot per position in the
 * comparison group (up to eight): PLAYER_SERIES from components/chart-kit.tsx,
 * the same eight-entry, dataviz-validated palette the Projections tab uses.
 * They are reused here as plain per-player accents rather than imported by
 * position, since this tab's players are not tied to position the way
 * Positional WAR's series are. A player's name is always adjacent visible
 * text, so this color is a decorative accent, never the only channel
 * carrying identity.
 */
function seriesFor(index: number): SeriesStyle {
  return PLAYER_SERIES[index % PLAYER_SERIES.length];
}
...
export function ReliabilityTab({
  ...
  const players: Side[] = sides.map((player) => ({
    player,
    reliability: extras.get(player.id)?.reliability ?? null,
  }));

  const anyWeeks = players.some((s) => (s.reliability?.weeks.length ?? 0) > 0);
  const anyGraded = players.some((s) => graded(s) !== null);
  ...
  if (!anyWeeks && !anyGraded) {
    return (
      <div className="space-y-4">
        <SectionHeading />
        <ChartEmpty>
          None of these players has enough graded games yet. Reliability needs projections and
          real results for the same weeks, so it fills in as the season is played.
        </ChartEmpty>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {/*
        One column on a phone, growing to more columns as width allows. Every
        card renders regardless of viewport; nothing is hidden at any
        breakpoint, the grid just reflows.
      */}
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {players.map((side, i) => (
          <ReliabilityCard key={side.player.slug} side={side} series={seriesFor(i)} />
        ))}
      </div>

      <DifferentialStrip players={players} />
      <BoomBustBars players={players} />
...
function ReliabilityCard({ side, series }: { side: Side; series: SeriesStyle }) {
  const { player } = side;
  const r = graded(side);
  ...
  return (
    <section
      ...
      // Per-player accent from PLAYER_SERIES rather than a fixed two-color
      // pair, since the group can hold up to eight players. ~40% opacity
      // border to match the border-line-weight the two-player version used.
      style={{ borderColor: `${series.color}66` }}
    >
...
/* ------------------------------------------------------------------ */
/* Actual minus projected, week by week, one strip per player          */
/* ------------------------------------------------------------------ */
...
function DifferentialStrip({ players }: { players: Side[] }) {
  const rows = players.filter((s) => comparable(s.reliability?.weeks ?? []).length > 0);
  if (rows.length === 0) return null;
  const season = chartSeasonOf(players);
  ...
  return (
    <ChartFigure
      ...
      description={`One mark per graded game${season ? ` in the ${season} season` : ""}, placed by how far above or below projection the player finished. A tight cluster near the middle line is a player you can set and forget. The tiles above cover every season we have graded, so their game counts are higher.`}
      summary={`${summary}.`}
      table={
        <DataTable
    ...
    >
      ...
      >
        {rows.map((s, idx) => {
          ...
          const series = seriesFor(players.indexOf(s));
          const weeks = comparable(s.reliability!.weeks);
          const scaleX = (d: number) => W / 2 + (d / bound) * (W / 2 - PAD_X);
          return (
            <g key={s.player.slug}>
              ...
              {weeks.map((w) => {
                ...
                return (
                  <path
                    key={w.week}
                    d={markerPath(series.marker, scaleX(d), y, 4)}
                    fill={series.color}
                    fillOpacity="0.75"
                  />
                );
...
function BoomBustBars({ players }: { players: Side[] }) {
  const rows = players
