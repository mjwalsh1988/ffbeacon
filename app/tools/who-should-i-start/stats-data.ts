...
/** Indices of the best value(s) among N players for one stat row, with a
 * small tolerance so identical numbers tie rather than arbitrarily picking
 * one. A missing (null) value is never best. Ties return every tied index,
 * so a genuine tie badges every tied cell rather than picking a winner. An
 * all-null row returns an empty set. */
export function bestIndices(
  ...
  for (const v of values) {
    if (v == null) continue;
    if (target == null || (better === "high" ? v > target : v < target)) {
      target = v;
    }
  }
  ...
  values.forEach((v, i) => {
    if (v != null && Math.abs(v - (target as number)) < TOL) out.add(i);
  });
  return out;
}

/** True when every player produced nothing for this stat (hide the row). */
export function allEmpty(values: ReadonlyArray<number | null>): boolean {
